...
- **Adopt it:** run `./inhabit.sh --repo <path> --identity <name>` — the one action that
  ...
  `--dispatch --launcher 'oc run'` for the local wake and `--github` for the cloud wake
  (gh-aw) — the same caretaker loop either way. To wake *your* agent on the log, follow
  `integrations/` — the contract is universal (AGENTS.md) and the hooks are batteries you
